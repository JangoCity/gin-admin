...
import (
	...
	"github.com/LyricTian/gin-admin/internal/app/module/rbac/model/entity"
...
func WrapPageQuery(db *gorm.DB, pp *gschema.PaginationParam, out interface{}) (*gschema.PaginationResult, error) {
	if pp != nil {
		total, err := gormplus.Wrap(db).FindPage(db, pp.PageIndex, pp.PageSize, out)
		if err != nil {
			return nil, err
		}
		return &gschema.PaginationResult{
			Total: total,
		}, nil
	}
